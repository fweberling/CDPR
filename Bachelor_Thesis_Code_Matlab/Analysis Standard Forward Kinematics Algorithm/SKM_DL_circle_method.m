% SKM Dog Leg with circle method (SKM DL circle method)

%Definition of Input Parameters for Model
%format long;

% generating object robot from the class Robot
robot=Robot;

% actuators' position in K_0
robot.a_1=[-0.6;-0.4;0];
robot.a_2=[0.6;-0.4;0];
robot.a_3=[-0.6;0.4;0];
robot.a_4=[0.6;0.4;0];
...
%pose of CDPR
robot.phi_x=0;% anterior tilt
robot.phi_y=0;% pelvic drop
...
% array with x- and y-components
x=-0.3:increment_step_x:0.3;
y=-0.3:increment_step_y:0.3;

% number of components f y
m=numel(y);
...
for iterations_1=1:m
    ...
    for iterations_2=1:robot.steps
        ...
        % Dog Leg (SKM) with circle method for inital estimate
        robot.L_SKM=inverse_kinematics_SKM(robot);
        estimated_position=circle_method(robot);
        robot.x_0=estimated_position(1); %starting point x-coordinate
        robot.y_0=estimated_position(2); %starting point y-coordinate
        robot.phi_0=0; %starting pint phi-coordinate
        robot.delta=0.5; %radius of trust region proposed 0.5
        robot.k_max=100; %maximum number of iterations
        robot.epsilon_1=10^-5; %proposed 10^-5
        robot.epsilon_2=10^-5; %proposed 10^-5
        robot.epsilon_3=10^-5; %proposed 10^-5
        ...
        position_accuracy_array(iterations_2)=...
            sqrt((X_DL(1)-x(1,iterations_2))^2 ...
            +(X_DL(2)-y(1,iterations_1))^2);
        
    end
    position_accuracy_matrix_real(iterations_1,:)=position_accuracy_array;
end

% position deviation matrix in micrometer 
position_deviation_matrix=10^6*abs(position_accuracy_matrix_ideal...
...
for iterations_1=1:m
    ...
    for iterations_2=1:robot.steps
        ...
        phi_accuracy_array(iterations_2)=X_DL(3);
...
for iterations_1=1:m
    ...
    for iterations_2=1:robot.steps
        ...
        robot.epsilon_2=10^-5; %proposed 10^-5
        robot.epsilon_3=10^-5; %proposed 10^-5
        [~,iterations_DL]=Dog_Leg_SKM(robot);
        
        
        iterations_array(iterations_2)=iterations_DL;
...
for iterations_1=1:m
    ...
    for iterations_2=1:robot.steps
        ...
        robot.y_0=estimated_position(2); %starting point y-coordinate
        robot.phi_0=0; %starting pint phi-coordinate
        [X_DL,iterations_DL]=Dog_Leg_SKM(robot);
...
for ii = 1:4;
    ...
    if ii==1
        ...
        colorbar_label=ylabel(c, '\Delta r [\mum]');
    ...
    if ii==3
        ...
        %subplot(2,2,3);
        contourf(X,Y,iterations_matrix,5);
...
matlab2tikz('SKM_DL_circle_method.tex');