% EKM Dog Leg with box method (EKM DL box method)
...
for iterations_1=1:m
    ...
    for iterations_2=1:robot.steps
        ...
        % Dog Leg Extended Kinematic Model (EKM) with box method for initial estimate
        [q_new,~,~]=inverse_kinematics_EKM(robot);
        robot.L_EKM=q_new;
        robot.L_SKM=inverse_kinematics_EKM(robot);
        estimated_position=box_method(robot);
        robot.x_0=estimated_position(1); %starting point x-coordinate
        robot.y_0=estimated_position(2); %starting point y-coordinate
        robot.phi_0=0; %starting pint phi-coordinate
        robot.delta=0.8; %radius of trust region proposed 0.8
        robot.k_max=100; %maximum number of iterations
        robot.epsilon_1=10^-5; %proposed 10^-5
        robot.epsilon_2=10^-5; %proposed 10^-5
        robot.epsilon_3=10^-5; %proposed 10^-5
...
for iterations_1=1:m
    ...
    for iterations_2=1:robot.steps
        ...
        [~,iterations_DL]= Dog_Leg_EKM(robot);
...
for iterations_1=1:m
    ...
    for iterations_2=1:robot.steps
        ...
        robot.y_0=estimated_position(2); %starting point y-coordinate
        robot.phi_0=0; %starting pint phi-coordinate
        [X_DL,iterations_DL]=Dog_Leg_EKM(robot);
...
matlab2tikz('EKM_DL_box_method.tex');