...
%pose of CDPR
robot.phi_x=pi/45;% anterior tilt
robot.phi_y=pi/45;% pelvic drop
robot.phi_z=pi/36;% rotation
...
for iterations_1=1:m
    ...
    for iterations_2=1:robot.steps
        robot.position=[x(1,iterations_2);y_vector;0.03];
...
for ii = 1:4;
    ...
    if ii==2
        ...
        %subplot(2,2,2);
        contourf(X,Y,orientation_deviation_matrix,1);
        c=colorbar;
    ...
    if ii==3
        ...
        %subplot(2,2,3);
        contourf(X,Y,iterations_matrix,10);
...
matlab2tikz('SKM_DL_circle_method_pelvic_usual.tex');
